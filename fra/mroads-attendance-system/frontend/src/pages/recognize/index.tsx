...
import { Users, Upload, Camera, Fingerprint } from 'lucide-react';

type RecognitionMethod = 'upload' | 'camera' | 'faceid';

const Recognize: React.FC = () => {
  const verificationRef = useRef<FaceVerificationRef>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const cameraInputRef = useRef<HTMLInputElement>(null);
  const [selectedMethod, setSelectedMethod] = useState<RecognitionMethod>('faceid');
  ...
  const handleComplete = (result: { matched: boolean; identity?: string; person?: any; matches?: any[] }) => {
    setMatchResult(result);
    setShowResult(true);
    setShowFaceID(false);

    // If matched, extract user info
    if (result.matched && result.person) {
      setUserInfo({
        name: result.person.name || result.identity || 'Unknown',
        email: result.person.email || '',
        id: result.person.id || result.person.user_id || '',
      });
    } else if (result.matched && result.identity) {
      setUserInfo({
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        const response = await fetch('http://localhost:9090/recognize', {
          method: 'POST',
        ...
        if (result.person || (result.matches && result.matches.length > 0)) {
          const hasKnownPerson = result.matches ? result.matches.some((m: any) => m.identity !== 'Unknown') : !!result.person;
          setMatchResult({
            matched: hasKnownPerson,
            identity: result.person?.name || result.identity,
    ...
    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="border-b border-stroke px-4 py-6 dark:border-strokedark sm:px-6">
          ...
          <p className="mt-2 text-sm text-bodydark">
            Position your face in front of the camera. The system will automatically identify you from the database.
          </p>
            </div>
        ...
        <div className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                >
                  ...
                  <span className="text-xs text-bodydark text-center">
                    Upload an image file to recognize
                  </span>
                </button>
                ...
                >
                  ...
                  <span className="text-xs text-bodydark text-center">
                    Use Face ID to recognize (optional)
                  </span>
                </button>
              </div>
          ...
          {/* Result Display (shown after recognition completes) */}
                ...
                {matchResult.matched ? (
                  <>
                    ...
                    <div className="flex-1">
                      <h3 className="text-lg font-semibold text-meta-3 mb-3">
                        {matchResult.matches && matchResult.matches.length > 1 ? 'Multiple Faces Recognized' : 'User Recognized'}
                      </h3>
                      {matchResult.matches && matchResult.matches.length > 0 ? (
                        <div className="space-y-3">
                          {matchResult.matches.map((match: any, idx: number) => (
                            <div key={idx} className={`p-4 rounded border ${match.identity !== 'Unknown' ? 'border-meta-3 bg-meta-3/10' : 'border-meta-1 bg-meta-1/10'} dark:bg-boxdark-2`}>
                              <h4 className="font-semibold text-black dark:text-white">Face {match.face_idx}: {match.identity}</h4>
                              {match.identity !== 'Unknown' && (
                                <p className="text-sm text-bodydark mt-1">
                                  <strong>User ID:</strong> {match.user_id || 'N/A'}<br/>
                                  <strong>Confidence:</strong> {(match.average_score * 100).toFixed(1)}%
                                </p>
                              )}
                              {match.identity === 'Unknown' && (
                                <p className="text-sm text-meta-1 mt-1">Not recognized in database.</p>
                              )}
                            </div>
                          ))}
                        </div>
                      ...
                      {matchResult.identity && !userInfo && (!matchResult.matches || matchResult.matches.length === 0) && (
                        <p className="text-sm text-black dark:text-white">
                    ...
                    <div className="flex-1">
                      ...
                      {matchResult.matches && matchResult.matches.length > 0 ? (
                        <div className="space-y-3 mt-3">
                          <p className="text-sm text-bodydark mb-2">Detected faces did not match any users.</p>
                          {matchResult.matches.map((match: any, idx: number) => (
                            <div key={idx} className="p-3 border rounded border-meta-1 bg-meta-1/10 dark:bg-boxdark-2">
                              <h4 className="font-semibold text-meta-1">Face {match.face_idx}: {match.identity}</h4>
                            </div>
                          ))}
                        </div>
                      ) : (
                        <p className="text-sm text-bodydark">
                          No face detected, or the face detected does not match any user in the database.
                        </p>
                      )}
                    </div>
                  </>
